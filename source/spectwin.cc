    ...
    _audio (audio), _input (-1), _drag (0), _p_ind (-1), _ngx (0), _ngy (0), _fftplan (0), _fftlen (64)
{
    ...
    p = xres->get (".minfreq", 0);
    if (! p || sscanf (p, "%d", &minfreq) != 1) minfreq = 0;
    _spect->_f0 = _f0 = minfreq;

    p = xres->get (".maxfreq", 0);
    if (! p || sscanf (p, "%d", &maxfreq) != 1) maxfreq = 8000;
    _spect->_f1 = _f1 = maxfreq;

     p = xres->get (".a4", 0);
    if (! p || sscanf (p, "%f", &a4) != 1) a4 = 440.0f;
    _a4 = a4;
}
...
{
    int n;
    float m = 12*log2(f/_a4)+69;
    int octave = (int)(m/12) - 1;
    f = fmodf(m+60, 12.0f);
    // f = fmodf (12 * log2 (f / _a4) + 129, 12.0f);
    n = (int)(floorf (f + 0.5f));
    f -= n;
    if (n == 12) n = 0;
    sprintf (s, "  %d%5s (%+2.0lf)", octave, _notes [n], 100.0f * f);
}
...
void Spectwin::calc_peak (float *f, float *p, float r)
{
    ...
    j = (int)(floor ((ff) * 2 * _fftlen / _fsamp + 0.5));
    n = (int)(5 * r); // xxx
    if (n < 2) n = 2;
    if (j < n) j = n;
